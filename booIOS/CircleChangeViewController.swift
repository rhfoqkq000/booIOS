//
//  CircleChangeViewController.swift
//  booIOS
//
//  Created by 정록헌 on 2017. 8. 2..
...
class CircleChangeViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        super.viewDidLoad()
        con.toastText("동아리 변경은 현재 소속된 동아리를 삭제한 뒤 진행합니다.")
    ...
    @IBAction func updateCircleButtonAction(_ sender: Any) {
        ...
        if rows != nil {
            ...
            //            print(user_id)
            postUpdateCircleJSON(user_id: user_id!,circles: news)
        } else {
    ...
    func postUpdateCircleJSON(user_id id:String,circles news:[Int]){
        let progressHUD = ProgressHUD(text: "로딩 중입니다...")
        self.view.addSubview(progressHUD)
        progressHUD.show()
        let todoEndpoint: String = "https://www.dongaboomin.xyz:20433/donga/updateCircle"
        //        let parameters = [
        //            "user_id": id,
        //            "news": news
        //        ]
        let parameters = ["user_id":id,"news":news] as [String : Any]
        let queue = DispatchQueue(label: "xyz.dongaboomin.updateCirlce", qos: .utility, attributes: [.concurrent])
        Alamofire.request(todoEndpoint, method: .post, parameters: parameters, encoding:JSONEncoding(options:[])).validate()
            ...
            .responseJSON(queue: queue,
                          completionHandler : { response in
                            ...
                            DispatchQueue.main.async {
                                progressHUD.hide()
                                self.con.toastText("동아리/학회 업데이트 성공!")
                            }
            }
        )
    }
